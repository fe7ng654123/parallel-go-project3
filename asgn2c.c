...
int get_p_points(Point *pts, int split_size, int dim, Point *p_result, int *p_num)
{
  ...
  memcpy(&p_result[(*p_num)++], &pts[0], sizeof(Point));

  for (int i = 1; i < split_size; i++)
  {
    int permissible = 1;
    int i_inserted_index = 0;
    int index = 0;

    Point *tmpPoints = malloc(split_size * sizeof(Point)); //for store updated P points
    for (int j = 0; j < *p_num; j++)
    {
      int counter = 0;
      
      for (int k = 0; k < dim; k++)
      { //compare with the known p_points first
        if (pts[i].values[k] >= p_result[j].values[k])
          counter++;
      }

      if (counter >= dim)
      { //all dimensions of point i are greater than or equal to point j
        permissible = 0;
        break;
      }
      else if (counter == 0)
      { //i preveils j, replace j with i
        if (i_inserted_index == 0)
        {
          memcpy(&tmpPoints[index], &pts[i], sizeof(Point));
          i_inserted_index = 1;
          index++;
        }
      }
      ...
      {
        memcpy(&tmpPoints[index], &p_result[j], sizeof(Point));
        index++;
      }
    }
    ...
    if (permissible)
    {
      ...
      else
      {
        ...
        // printf("replace pointer! insert ID = %d\n", i+1);
        // p_result = tmpPoints;
        memcpy(p_result, tmpPoints, sizeof(Point)*index);
        free(tmpPoints);
        (*p_num) = index;
      }
    }
...
int main(int agrc, char *argv[])
{
  ...
  for (int i = 0; i < permissiblePointNum; i++)
  {
    ...
    // printf("rank %d : p_result[%d].ID = %d\n",world_rank,i,p_result[i].ID);
  }
